...
public class UserInfo {
    ...
    public static void saveUserProfilePic(@NonNull Context context, @NonNull Uri uri, @Nullable TaskRunner.Callback<String> callback) {
        ...
        TaskRunner.executeAsync(() -> {
            ...
            if (null != callback) handler.post(() -> callback.onComplete(file.getAbsolutePath()));
        });
    }

    public static String getUserProfilePic(Context context) {
        return new File(context.getFilesDir().getAbsolutePath(), PROFILE_PICTURE).getAbsolutePath();
    }
}